...
public class XmlHelperTest extends TestCase {
  ...
  public void testEncodeDecode() {
    ...
    Assert.assertEquals("Error encoding after decoding", encodedXml, XmlHelper.encode(XmlHelper.decode(encodedXml))); //$NON-NLS-1$
  ...
  public void testFailureTransformXML() {
    try {
      ...
      Document document = XmlDom4JHelper.getDocFromString(
          "<?xml version=\"1.0\" encoding=\"windows-1252\"?><root></root>", null); //$NON-NLS-1$
  ...
  public void testFailureTransformXML2() {
    try {
      ...
      String xmlString = "<?xml version=\"1.0\" encoding=\"" + "UTF-8" + "\" ?><filters xmlns:xf=\"http://www.w3.org/2002/xforms\">" + //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$  
  ...
  public void testFailureTransformXML3() {
    try {
      ...
      StringBuffer content = XmlHelper.transformXml(parameterXsl, xslPath, new ByteArrayInputStream(document.asXML()
          .getBytes()), parameters, new JarEntityResolver());
      System.out.println("Transformed XML" + content); //$NON-NLS-1$
    } catch (Exception e) {
      e.printStackTrace();
      Assert.assertTrue("Exception thrown " + e.getMessage(), false); //$NON-NLS-1$
