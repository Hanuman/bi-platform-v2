...
public class CacheManagerWithRegionTest extends BaseTest {
    ...
    Assert.assertNull(user2CachedDocument);
		...
		// Get them back out
		Object user1CachedStringObject1 = cacheManager.getFromRegionCache(
				userSession1.getId(), "StringObject"); //$NON-NLS-1$
		...
		Object user2CachedStringObject1 = cacheManager.getFromRegionCache(
				userSession2.getId(), "StringObject"); //$NON-NLS-1$
		...
		Object notThere = cacheManager.getFromRegionCache(userSession1.getId(),
				"StringObject"); //$NON-NLS-1$
		Assert.assertNull(notThere);

		// Make sure that User2 is unaffected
		Object shouldBeThere = cacheManager.getFromRegionCache(userSession2.getId(),
				"StringObject"); //$NON-NLS-1$
		...
		shouldBeThere = cacheManager.getFromRegionCache(userSession2.getId(),
				"StringObject"); //$NON-NLS-1$
		Assert.assertNotNull(shouldBeThere);
		...
		notThere = cacheManager.getFromRegionCache(userSession2.getId(),
				"StringObject"); //$NON-NLS-1$
		Assert.assertNull(notThere);
	...
	public static void main(final String[] args) {
	  CacheManagerWithRegionTest test = new CacheManagerWithRegionTest();
		try {
			test.testCacheRegion();
		} finally {
		}
