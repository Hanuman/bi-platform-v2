...
public class PluginClassLoaderTest {
  ...
  public void init() {
    // now load a class
    //    pluginLoader = new PluginClassLoader( new File("./plugin-mgr/test-res/plugin-classloader-test/"), getClass().getClassLoader() ); 
    pluginLoader = new PluginClassLoader(new File("./plugin-mgr/test-res/PluginClassLoaderTest/"), ClassLoader
  ...
  private Object getContainerFromPrivateClassLoader() throws ClassNotFoundException, InstantiationException, IllegalAccessException {
  ...
  public void testImplicitLoad() throws ClassNotFoundException, InstantiationException, IllegalAccessException {
    ...
    assertEquals(PluginClassLoader.class.getName(), explicitlyLoadedContainer.getClass().getClassLoader().getClass().getName());
    assertEquals("failed to load override class", "I am the overridden class from the plugin class loader", explicitlyLoadedContainer
        .toString());
  }
  ...
  public void testLoadClassAsResource() throws IOException, ClassNotFoundException {
    ...
    InputStream in = pluginLoader
        .getResourceAsStream("org/pentaho/test/platform/engine/services/TestClassForClassloader.class");
