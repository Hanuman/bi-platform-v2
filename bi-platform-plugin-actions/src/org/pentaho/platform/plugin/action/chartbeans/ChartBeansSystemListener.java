...
public class ChartBeansSystemListener implements IPentahoSystemListener {
	...
	/**
	 * This methods looks in the chartbeans configuration file, and retrieves the 
	 * prescribed list of chart plugins (renderers). For each defined renderer, 
	 * the method then looks to the PluginManager to see if that renderer has 
	 * been overridden. If it has, then the class defined in the PluginManager 
	 * will be loaded IN PLACE OF the configuration file's class.  
	 * 
	 * @return list of available chart "plugin" (renderer) instances.
	 * @throws Exception if no chart plugins (renderers) are found.
	 */
