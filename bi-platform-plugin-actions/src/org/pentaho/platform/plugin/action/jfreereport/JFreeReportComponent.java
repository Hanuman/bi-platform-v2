 ...
 * process. <p/> This component will execute JFreeReport reports in one of two
 * different modes: <p/>
 * <h3>sqlMode = true</h3>
 * <p/> This means that the JFreeReport component is expected to execute an SQL query,
 ...
 * the jfreereport xml document. <p/> In the component-definition sction, there must
 ...
 * <li>or The database parameters "driver", "user-id", "password" and "connection" so
 ...
 * </ul>
 * <h3>sqlMode = false</h3>
 * <p/> This means that the JFreeReport component is expected to execute a report that
 ...
 * contains the report .xml file, and the TableModel implementation. <p/> In the
 * component-definition section, there must be two entries:
 * <ul>
 * <li> "report-location" - This is the location of the report .xml document (e.g.
 * org/jfree/report/demo/report1.xml)</li>
 * <li> "class-location" - This is the package-qualified class that implements
 * TableModel (e.g. org.jfree.report.demo.SampleData1).</li>
 * </ul>
 * 
 * @author mbatchel
 * @created Sep 8, 2005
...
public class JFreeReportComponent extends AbstractJFreeReportComponent {
  ...
  public boolean executeAction() {
    ...
    if (initParamsResult == JFreeReportComponent.INIT_REPORT_PARAMS_STATUS_FAILED) {
      result = false;
    } else if ((initParamsResult == JFreeReportComponent.INIT_REPORT_PARAMS_STATUS_PROMPT_PENDING) || isPromptPending()) {
      result = true;
  ...
  private PentahoTableDataFactory getQueryComponentDataFactory() throws ClassNotFoundException, InstantiationException,
    ...
    if (dataComponentName != null) {
      ...
      try {
        ...
        // SQL or MDX lookup action, by changing its component name. Then we create the appropriate component to run the transformed action.
        // All this to support the DB and Query info being embedded in the JFreeReport action. This is definitely deprecated functionality
        // that should not be relied upon. The correct way to do this is to create an SQL or MDX action prior to the JFreeReport
        // action in the action sequence. That action performs the desired query, then pass the results of that query to the JFreeReport
        // action.
        jFreeReportAction.setComponentName(dataComponentName);
        ActionDefinition tmpActionDefinition = ActionFactory.getActionDefinition(jFreeReportAction.getElement(),
            jFreeReportAction.getActionParameterMgr());
        ...
        try {
          ...
          if (in.read() == -1) {
            ...
            if (loader == null) {
              ...
              final TableModel model = dataFactory.queryData(jFreeReportAction.getDataJar().getDataClass(),
                  new ParameterDataRow(reportProperties));
  ...
  private void setReportConfigParameters(final MasterReport report,
    ...
    for (int i = 0; i < reportConfig.size(); i++) {
      JFreeReportAction.StaticReportConfigItem staticReportConfigItem = (JFreeReportAction.StaticReportConfigItem) reportConfig
          .get(i);
  ...
  private boolean generateReport(final MasterReport report, final PentahoTableDataFactory factory) throws IOException {
    ...
    if (printerName != null) {
      ...
      result = writeReport(outputFormat, report, outputStream, jFreeReportAction.getReportGenerationYieldRate()
