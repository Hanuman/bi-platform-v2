...
public class JFreeReportSubReportsTest extends BaseTest {
  ...
  public void testJFreeReportSubReport() {
    ...
    IRuntimeContext context = run(
        "samples", "reporting", "subreport-example.xaction", null, false, parameterProvider, outputHandler, session); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
    assertEquals(
  ...
  public void testJFreeReportSubReportBasicHTML() {
    ...
    StandaloneSession session = new StandaloneSession(Messages.getString("BaseTest.DEBUG_JUNIT_SESSION")); //$NON-NLS-1$
    IRuntimeContext context = run(
        "test", "reporting", "jfreereport-subreport-basic-test.xaction", null, false, parameterProvider, outputHandler, session); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
    assertEquals(
        Messages.getString("BaseTest.USER_RUNNING_ACTION_SEQUENCE"), IRuntimeContext.RUNTIME_STATUS_SUCCESS, context.getStatus()); //$NON-NLS-1$
    // TODO need some validation of success
    finishTest();
  ...
  public void testJFreeReportSubReportIPreparedComponentHTML() {
    ...
    StandaloneSession session = new StandaloneSession(Messages.getString("BaseTest.DEBUG_JUNIT_SESSION")); //$NON-NLS-1$
    IRuntimeContext context = run(
        "test", "reporting", "jfreereport-subreport-ipreparedcomponent-test.xaction", null, false, parameterProvider, outputHandler, session); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
    assertEquals(
        Messages.getString("BaseTest.USER_RUNNING_ACTION_SEQUENCE"), IRuntimeContext.RUNTIME_STATUS_SUCCESS, context.getStatus()); //$NON-NLS-1$
    // TODO need some validation of success
    finishTest();
