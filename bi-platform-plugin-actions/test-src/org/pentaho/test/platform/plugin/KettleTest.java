...
public class KettleTest extends BaseTest {
  ...
  public String getSolutionPath() {
    ...
    public void testKettle() {
        ...
        IRuntimeContext context = run("test", "etl", "SampleTransformation.xaction", null, false, parameterProvider, outputHandler, session); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
    ...
    public void testKettleValidatationFailure() {
      ...
      StandaloneSession session = new StandaloneSession(Messages.getString("BaseTest.DEBUG_JUNIT_SESSION")); //$NON-NLS-1$
      assertNotNull(session);
      IRuntimeContext context = run("test", "etl", "SampleTransformationInvalid.xaction", null, false, parameterProvider, outputHandler, session); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
    ...
    public void testKettleMissingTransform() {
      ...
      StandaloneSession session = new StandaloneSession(Messages.getString("BaseTest.DEBUG_JUNIT_SESSION")); //$NON-NLS-1$
      assertNotNull(session);
      IRuntimeContext context = run("test", "etl", "SampleTransformationMissingKTR.xaction", null, false, parameterProvider, outputHandler, session); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
