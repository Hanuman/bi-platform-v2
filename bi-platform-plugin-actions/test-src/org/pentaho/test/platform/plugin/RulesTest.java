...
public class RulesTest extends BaseTest {
  ...
  public void testScriptRule() {
    ...
    assertNotNull(Messages.getString("RulesTest.ERROR_0001_NULL_RESULT"), context.getOutputParameter("rule-result")); //$NON-NLS-1$ //$NON-NLS-2$
    assertEquals(
        Messages.getString("RulesTest.ERROR_0002_WRONG_RESULT"), "Central", context.getOutputParameter("rule-result").getStringValue()); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
    info(Messages.getString(
  ...
  public void testScriptRuleError1() {
    startTest();
    info(Messages.getString("RulesTest.USER_ERRORS_EXPECTED_SCRIPT_NOT_DEFINED")); //$NON-NLS-1$
    ...
    assertNotNull(
        Messages.getString("RulesTest.ERROR_0004_NULL_OUTPUT_OBJECT"), context.getOutputParameter("rule-result")); //$NON-NLS-1$//$NON-NLS-2$
    assertEquals(
        Messages.getString("RulesTest.ERROR_0005_RESULT_WHEN_NULL_EXPECTED"), null, context.getOutputParameter("rule-result").getStringValue()); //$NON-NLS-1$//$NON-NLS-2$
    finishTest();
  }

  public void testScriptRuleError2() {
    startTest();
    info(Messages.getString("RulesTest.USER_ERRORS_EXPECTED_OUTPUT_NOT_DEFINED")); //$NON-NLS-1$
    IRuntimeContext context = run("test", "rules", "script_rule_error2.xaction"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
    assertEquals(
        Messages.getString("BaseTest.USER_RUNNING_ACTION_SEQUENCE"), IRuntimeContext.RUNTIME_CONTEXT_VALIDATE_FAIL, context.getStatus()); //$NON-NLS-1$

    assertEquals(
        Messages.getString("RulesTest.ERROR_0006_RESULT_WHEN_ERROR_EXPECTED"), false, context.getOutputNames().contains("rule-result")); //$NON-NLS-1$//$NON-NLS-2$
    finishTest();
  }

  public void testScriptRuleError3() {
    startTest();
    info(Messages.getString("RulesTest.USER_ERRORS_EXPECTED_SCRIPT_INVALID")); //$NON-NLS-1$
  ...
  /*public void testQueryRule() {
    ...
    try {
      ...
      assertNotNull(Messages.getString("RulesTest.ERROR_0001_NULL_RESULT"), result); //$NON-NLS-1$

      assertTrue(
          Messages.getString("RulesTest.ERROR_0007_LOOKUP_RULE_INVALID_RESULT"), (result instanceof IPentahoResultSet)); //$NON-NLS-1$
      ...
      assertEquals(Messages.getString("RulesTest.ERROR_0009_LOOKUP_RULE_COLUMN_MISSING"), "POSITIONTITLE", columnHeader); //$NON-NLS-1$ //$NON-NLS-2$

      Object row[] = resultSet.next();
      assertNotNull(Messages.getString("RulesTest.ERROR_0007_LOOKUP_RULE_INVALID_RESULT"), row); //$NON-NLS-1$

      info(Messages.getString("RulesTest.DEBUG_LOOKUP_RULE_SUCCESS", row[0].toString())); //$NON-NLS-1$
  ...
  /*public void testQueryRule2() {
    ...
    try {
      ...
      assertEquals(Messages.getString("RulesTest.ERROR_0009_LOOKUP_RULE_COLUMN_MISSING"), "REGION", regionHeader); //$NON-NLS-1$ //$NON-NLS-2$
      assertEquals(
          Messages.getString("RulesTest.ERROR_0009_LOOKUP_RULE_COLUMN_MISSING"), "DEPARTMENT", departmentHeader); //$NON-NLS-1$ //$NON-NLS-2$
      ...
      while (row != null) {
        ...
        assertNotNull(Messages.getString("RulesTest.ERROR_0009_LOOKUP_RULE_COLUMN_MISSING"), region); //$NON-NLS-1$
        assertNotNull(Messages.getString("RulesTest.ERROR_0009_LOOKUP_RULE_COLUMN_MISSING"), department); //$NON-NLS-1$
