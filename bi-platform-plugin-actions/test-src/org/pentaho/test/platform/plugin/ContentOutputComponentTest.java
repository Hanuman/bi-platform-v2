...
public class ContentOutputComponentTest extends BaseTest {
  ...
  /*public void testErrorPaths() {
    ...
    SimpleParameterProvider parameterProvider = new SimpleParameterProvider();
    parameterProvider.setParameter("CONTENTOUTPUT", neverWritten); //$NON-NLS-1$
    IRuntimeContext context = run(
        "test", "platform", "ContentOutputTest_Bytearray.xaction", parameterProvider, testName, CO_TEST_EXTN); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
    assertEquals(
    ...
    parameterProvider.setParameter("CONTENTOUTPUT", baos); //$NON-NLS-1$
    context = run("test", "platform", "ContentOutputTest_Bytearray.xaction", parameterProvider, testName, CO_TEST_EXTN); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
    String newText = FileHelper.getStringFromInputStream(null);
    System.out.println("Null Input Stream" + newText);//$NON-NLS-1$
