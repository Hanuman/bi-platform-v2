...
:gotJreHome
REM if not exist "%JRE_HOME%\bin\java.exe" goto noJavaHome
REM if not exist "%JRE_HOME%\bin\javaw.exe" goto noJavaHome
if not exist "%JRE_HOME%\lib\tools.jar" goto noJreToolsJar
set CLASSPATH=%JRE_HOME%\lib\tools.jar
...
if not exist "%JAVA_HOME%\lib\tools.jar" goto noToolsJar
...
goto exit
:noJreToolsJar
echo The file %JRE_HOME%/lib/tools.jar 
echo cannot be found and is required.
goto exit
:noToolsJar
echo The file %JAVA_HOME%/lib/tools.jar 
echo cannot be found and is required.
goto exit
...
REM Removed this definition of classpath; added the two definitions
REM above that use either JAVA_HOME or JRE_HOME.
