...
public interface ICacheManager extends ILogoutListener {
   ...
   * goes away. This performs required cleanup in the underlying cache
   * implementation. In some cases, failure to stop the cache will cause
   * cached items saved to disk to be un-recoverable.
   * 
   */
  public void cacheStop();
  /**
   * Removes any session-based data for the specified
   * <code>IPentahoSession</code>.
   * 
   * @param session
  ...
  /**
   * Puts an object in the session-specific cache. The session specified must
   * have a valid session id.
   * <p>
   ...
   * <code>UUIDUtil</code> to generate a unique sessionId for each
   * standalone session.
   * 
   * @param session
  ...
  /**
   * Gets an object from the user session specific cache. If the object
   * doesn't exist in the cache, null is returned.
   * 
   * @param session
   *            The users IPentahoSession
   * @param key
  ...
  /**
   * Puts an object directly into the cache without translating the passed in
   * key.
   * <p>
   * Important note - most cache implementations require both the key and the
   * value to be serializable.
   * <p>
   * 
   * @param key
   ...
   * 
   * @param region   The region whose objects needs to be removed from the cache.
   */  
