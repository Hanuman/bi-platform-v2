 ...
 * action execution to resolve parameters, inputs and outputs, resources,
 * and persist the runtime data. Think of the runtime context as
 * working storage for the execution of an action.
 * 
 * @author James Dixon
 */
public interface IRuntimeContext extends IAuditable, ILogger {
  ...
  /**
   * Indicates that the parameters for an action were
   * able to be resolved.
   */
  public static final int RUNTIME_CONTEXT_RESOLVE_OK = 2;

  /**
   * Indicates that parameters for an action could not
   * be resolved.
   */
  public static final int RUNTIME_CONTEXT_RESOLVE_FAIL = 9;

  /**
   * When beginning execution of an action sequence, the
   * status starts off as not started.
   */
  ...
  /**
   * As an action sequence begins execution, the status is
   * set to running.
   */
  ...
  /**
   * Currently only used as an indicator that BIRT report specification parameters
   * could not be read.
   */
  ...
  /**
   * Indicates that parameters are required by an action, and the parameters aren't
   * available so they need to be prompted for.
   */
  ...
  /**
   * Returns the unique context identifier. The handle is created during
   * construction of the <code>RuntimeContext</code> object, and should be
   * unique down to the date/time of construction. The default form of this
   * as implemented in <tt>RuntimeContext</tt> includes context- plus the
  ...
  /**
   * Returns the unique execution instance. This is typically a GUID that can be used to
   * track the entire execution of an action sequence all the way from beginning to end.
   ...
   * @param actionSequence The action sequence to validate
   */
   ...
   * @param listener The listener to be notified when the sequence finishes
   * @param async Whether the action is synchronous or asynchronous.
  ...
  /**
   * @return The name of the solution holding the currently executing action sequence
   */
   ...
   * @param messageType Message type as defined in <tt>MessageTypes</tt>
   * @param message Message to appear in the log
   * @param value Value of an object to be logged.
   * @param duration For time-tracked execution paths, indicates the duration the task took
   ...
   * (<tt>IParameterProvider</tt>) in order until it finds the input parameter and returns it.
   * Throws a <tt>NullPointerException</tt> if the parameter is not found. This method never returns
   * <code>null</code>
   ...
   * (<tt>IParameterProvider</tt>) in order until it finds the output parameter and returns it.
   * Throws a <tt>NullPointerException</tt> if the parameter is not found. This method never returns
   * <code>null</code>
   * @param name The name of the parameter to get
   ...
   * Throws a <tt>NullPointerException</tt> if the resource is not found. This method never returns
   * <code>null</code>
   * @param name The named resource to get
   ...
   * Throws a <tt>NullPointerException</tt> if the parameter is not found. This method never returns
   * <code>null</code>
   * @param name The named parameter to retrieve
  ...
  /**
   * Gets the value of the specified input parameter as a <code>String</code>.
   * Throws a <tt>NullPointerException</tt> if the parameter is not found. This method never returns
   * <code>null</code>
   * @param name The named parameter to retrieve
  ...
  /**
   * Gets the named resource as a <tt>Reader</tt>. This is just a utility
   * method that interfaces to <tt>ISolutionRepository</tt>
   * @param actionResource The resource to get from the <code>SolutionRepository</tt>
   * @return The <code>Reader</code>
   * @see ISolutionRepository#getResourceReader(IActionResource)
   */
   ...
   * @param output The value to set the output parameter to
   */
   ...
   * @param output The value to set the temporary parameter to
   */
   ...
   * @param extension The file extension of the output
   * @return OutputStream for writing to
   */
  public IContentItem getOutputItem(String outputName, String mimeType, String extension);

  /**
   * Returns an input stream from an input parameter, if the input parameter is
   * a content item.
  ...
  /**
   * Does parameter substitution on the input string, searching for all parameter declarations in the input
   * string, and substituting the value from the matching input parameter. In other words, it replaces 
   * {REGION} with the value of the input parameter called REGION.
   * @param format The string containing possible parameter references
   ...
   * {REGION} with the value of the input parameter called REGION.
   * @param format The string containing possible parameter references
   * @param Resolver for parameters for overriding behavior
   * @return String with parameters resolved.
   */
   ...
   * @param name The name of the parameter (the key to the parameter map)
  ...
  /**
   * @return true if the current output device allows 
   * user feedback (i.e. parameter input forms)
   */
  public boolean feedbackAllowed();

  /**
   * Interfaces to the current output handler to get the content item
   * that is handling feedback (i.e. parameter input forms)
  ...
  /**
   * Interfaces to the current output handler to get the content item
   * that describes the output from this request's component execution.
  ...
  /**
   * Interfaces to the current output handler to get the named content item
   * from this request's component execution.
   * @param outputName the name of the output
  ...
  /**
   * Adds a feedback parameter (essentially a form input field) for a
   * required input. 
   * @param selMap Maps <code>IPentahoResultSet</code> objects to selection objects
   * @param fieldName Name of the form field
   * @param defaultValues default values for the input field
   * @see SelectionMapper
   */
   ...
   * @param fieldName Name of the form field
   * @param defaultValues default values for the input field
   * @param optional specifies if the feedback parameter is required or not
   * @see SelectionMapper
   */
   ...
   * @param displayName display name of the input field
   * @param hint Fly-over hint for the input field
   * @param defaultValue Default value for the input field
   * @param visible Whether the input field is visible or not
   * @see XForm
   */
   ...
   * @param hint Fly-over hint for the input field
   * @param defaultValues Default value of the input field
   * @param values List of values 
   * @param dispNames Map of display names
   ...
   * attributes of the runtime data from the parameter Map
   * 
   * @param persisted
   *            Should the runtime data be persisted
   * @param parameters
  ...
  /**
   * Sets the target window that the content will be displayed in. This name
   * is used at the target in an Window.open() javascript call made when the
   * submit button on the parameter page is clicked.
   * 
   * @param target
   *            Window name
   */
