...
abstract public class FilterDefinition {
  ...
  /**
   * a resultSet containing the text and values for the items to be displayed in the filter panel's
   * control. The result set may be a scoped variable, in which case it is retrieved by the key
   * specified in the <global-attribute> or <session-attribute> element in the filter panel
   * definition file. Or, it is the output of the action sequence, in which case it is retrieved
   * from the action sequence's output specified by the name of the <data-output> element
   * of the filter panel definition file.
   */
  private IPentahoResultSet resultSet;

  /**
   * 
   * the type of the control in the filter panel, identified by the element
   * <type> in the filter panel definition file. Valid values are:
   * radio,list,list-multi,check-multi,check-multi-scroll,check-multi-scroll-2-column,
   * check-multi-scroll-3-column,check-multi-scroll-4-column
   */
  private int type;

  /**
   * name of the column to retrieve the names of the items placed in the filter panel's control
   * related to the member variable nameColumnNo
   */
  protected String descriptionItem;

  /**
   * name of the column to retrieve the values of the items placed in the filter panel's control
   * related to the member variable valueColumnNo
   */
  ...
  /**
   * index of the column to retrieve the names of the items placed in the filter panel's control
   * related to the member variable descriptionItem
   */
  private int nameColumnNo = -1;

  /**
   * index of the column to retrieve the values of the items placed in the filter panel's control
   * related to the member variable valueItem
   */
  ...
  /*    private boolean isValid(String[] value, Map parameterProviders) {
   if (resultSet == null && parameterProviders != null) {
   populate(parameterProviders, value);
   }
  ...
  /**
   * Create the XForm header and XForm body, and place the results in the
   * parameters xformHeader and xformBody.
   * 
   * @param xformHeader
   ...
   *            the <data-display> element in the filter panel definition
   *            file and the name of a column in the resultSet.
   *            valueColumnNo is likely to be -1 if this.valueItem does not
   *            correlate with the value of the <data-value> element in
   *            the filter panel definition file and the name of a column in
   *            the resultSet. resultSet is likely to be null if a
   *            result-set was not placed in session or global scope under
   *            the key identified by the <global-attribute> or
   *            <session-attribute> element in the filter panel definition
   *            file, or if the action sequence identified by <data-action>
   *            element in the filter panel definition file failed to return
   *            a result set
   */
