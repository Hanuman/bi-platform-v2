...
public class CategoryDatasetChartComponent extends AbstractJFreeChartComponent {
  ...
  public Document getXmlContent() {
    ...
    try {
      chartDefinition = AbstractJFreeChartComponent.getResourceAsDocument(getSession(), resource);
    } catch (IOException e) {
    }
  ...
  private void populateCategoryItemEntity(CategoryItemEntity categoryItemEntity, List seriesListArray) {
    ...
    if (paramName == null) {
      ...
      try {
        ...
        encodedVal = Messages.getString("CategoryDatasetChartComponent.UNKNOWN_SERIES"); //$NON-NLS-1$
