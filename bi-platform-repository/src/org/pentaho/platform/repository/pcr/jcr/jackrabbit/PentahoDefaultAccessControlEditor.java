...
public class PentahoDefaultAccessControlEditor extends PentahoProtectedItemModifier implements AccessControlEditor,
    AccessControlConstants {

  /**
   * the default logger
   */
  private static final Logger log = LoggerFactory.getLogger(PentahoDefaultAccessControlEditor.class);
  ...
  PentahoDefaultAccessControlEditor(Session editingSession, AccessControlUtils utils) {
    super(Permission.MODIFY_AC);
    if (editingSession instanceof SessionImpl) {
      systemSession = ((SessionImpl) editingSession);
  ...
  public AccessControlPolicy[] editAccessControlPolicies(String nodePath) throws AccessControlException,
    ...
    if (aclNode == null) {
      ...
      String mixin = systemSession.getJCRName(NT_REP_ACCESS_CONTROLLABLE);
      if (controlledNode.isNodeType(mixin) || controlledNode.canAddMixin(mixin)) {
        acl = new PentahoJackrabbitAccessControlList(nodePath, systemSession.getPrincipalManager(), privilegeRegistry,
  ...
  public JackrabbitAccessControlPolicy[] editAccessControlPolicies(Principal principal) throws AccessDeniedException,
    ...
    if (!systemSession.getPrincipalManager().hasPrincipal(principal.getName())) {
  ...
  private NodeImpl getNode(String path) throws PathNotFoundException, RepositoryException {
    return (NodeImpl) systemSession.getNode(path);
  }
  ...
  private NodeImpl getAclNode(NodeImpl controlledNode) throws RepositoryException {
    ...
    if (AbstractPentahoAccessControlProvider.isAccessControlled(controlledNode)) {
