  ...
  Tree bookmarkTree = new Tree(MantleImages.bookmarkImages) {
    ...
    public void onBrowserEvent(Event event) {
      ...
      if (DOM.eventGetButton(event) == Event.BUTTON_RIGHT && getSelectedItem().getUserObject() instanceof Bookmark) {
        ...
        menuBar.addItem(new MenuItem(Messages.getString("edit"), new Command() { //$NON-NLS-1$
          public void execute() {
          }
        }));
        menuBar.addItem(new MenuItem(Messages.getString("delete"), new Command() { //$NON-NLS-1$
          public void execute() {
          }
        }));
